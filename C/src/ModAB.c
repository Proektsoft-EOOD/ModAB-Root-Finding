...
static inline bool same_nonzero_sign(double x, double y) {
...
static inline bool passes_switching_test(double ym, double yf, double symmetry) {
    ...
    double scale = fmax(abs_yf, abs_ym);
    double norm_ym = ym / scale;
    double norm_yf = yf / scale;
...
EXPORT double modAB_find_root(double (*f)(double), double x1, double x2, double aTol, double rTol, int maxIter) {
    ...
    for (int i = 1; i <= maxIter; ++i) {
        ...
        if (bisection) {
            y3 = eval(f, x3);
            double ym = safe_midpoint(f1, f2); // Ordinate of chord at midpoint
