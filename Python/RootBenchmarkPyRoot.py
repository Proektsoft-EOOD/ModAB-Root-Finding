...
from pyroot import solver as pr_solver
from dataclasses import dataclass
from typing import Callable

# Benchmark of pymodab against the bracketed solvers of PyRoot
# (https://github.com/SimpleArt/pyroot). Both packages are used as
# installed from PyPI:  pip install pymodab==1.0.7 pyroot==0.2.0
# PyRoot 0.3.1 (the latest release) is not used, because its solvers
# do not converge on most of the test problems.
...
def make_pyroot_solver(method, name):
    """Create a wrapper for pyroot.solver that matches the (f, a, b, precision) signature."""
    def solver(f, a, b, precision=1e-14):
        try:
            return pr_solver(f, a, b, method=method, x_err=precision, r_err=precision, refine=0)
        except (ValueError, RuntimeError, ZeroDivisionError, OverflowError):
            return float('nan')
    solver.__name__ = name
    return solver

pr_bisect = make_pyroot_solver("bisection",          "pr_bisection")
pr_secant = make_pyroot_solver("secant",             "pr_secant")
pr_dekker = make_pyroot_solver("dekker",             "pr_dekker")
pr_brent  = make_pyroot_solver("brent",              "pr_brent")
pr_chandr = make_pyroot_solver("chandrupatla",       "pr_chandrupatla")
pr_chmix  = make_pyroot_solver("chandrupatla mixed", "pr_chandrupatla_mixed")
