...
# Import the three solvers to compare
import cymodab
from cybrentq import brentq as cybrentq_brentq
...
def cymodab_solver(f, a, b, precision=1e-14):
    """Wrapper for cymodab.modAB_root."""
    try:
        return cymodab.modAB_root(f, a, b, xtol=precision)
    except (ValueError, RuntimeError):
        return float('nan')


def pymodab_solver(f, a, b, precision=1e-14):
    """Wrapper for pymodab.find_root."""
    try:
...
def cybrentq_solver(f, a, b, precision=1e-14):
    """Wrapper for cybrentq.brentq."""
    try:
        return cybrentq_brentq(f, a, b, xtol=precision, rtol=precision, disp=False)
    except (ValueError, RuntimeError):
        return float('nan')
...
# Solver table - cymodab is last for the relative-factor column
solvers = [
    ("cybrentq", cybrentq_solver),
    ("pymodab", pymodab_solver),
    ("cymodab", cymodab_solver),
]

MODAB_IDX = len(solvers) - 1  # index of cymodab in the solvers list
...
def _print_stats_block(data, col_w, solver_names, *, integer_vals=False):
    """
    Print avg / median / min / max / factor-vs-cymodab summary rows for one metric.
    ...
    # Relative factor vs modab_cy (modab_cy = 1.00x baseline)
...
def run():
    ...
    print("Execution times  (ms per problem, 200 iterations)")
    print("===")
    iterations = 200
    header = f"{'Func':>6}| " + "| ".join(f"{name:>{cnt_w}}" for name, _ in solvers) + "|"
    print(header)
